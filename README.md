...
Vercel deployments include the `api/voice-review.js` serverless endpoint, so no frontend endpoint variable is required. Add `ELEVENLABS_API_KEY` in the Vercel project’s Environment Variables settings for Production, Preview, and Development as needed. The key is read only by the serverless endpoint; never expose it in a `VITE_` variable. You may set `ELEVENLABS_VOICE_ID` to override the built-in Adam voice (`pNInz6obpgDQGcFmaJgB`). Use **Test voice** once after opening the app so the browser permits audio playback.
